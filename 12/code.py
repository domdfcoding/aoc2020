...
The ferry needs to take evasive action!

Unfortunately, the ship's navigation computer seems to be malfunctioning;
rather than giving a route directly to safety,
it produced extremely circuitous instructions.
...
the ship would move north ``10`` units,
...
- ``F10`` would move the ship ``10`` units east
  (because the ship starts by facing east)
  to east ``10``, north ``0``.
...
for instruction in instructions:
	m = re.match(r"([NSEWLRF])([0-9]+)", instruction)
	if m:
		direction = m.group(1)
	...
	if direction == 'E':
		...
		if amount % 90:
			raise ValueError(f"Incompatible angle {amount}")
		angle += amount
		angle %= 360
	elif direction == 'L':
		if amount % 90:
	...
	# print(instruction, x_pos, y_pos, angle)
	# input(">>> ")

manhattan_distance = abs(x_pos) + abs(y_pos)

print("The manhattan distance from the start is:", manhattan_distance)  # 582
...
you realize that the actual action meanings were printed on the
...
- ``N3`` moves the waypoint ``3`` units north to ``10`` units east
  and ``4`` units north of the ship. The ship remains at east ``100``, north ``10``.
...
for instruction in instructions:
	...
	# print(instruction, x_pos, y_pos, w_x_pos, w_y_pos)
	# input(">>> ")

manhattan_distance = abs(x_pos) + abs(y_pos)

print("The manhattan distance from the start is:", manhattan_distance)  # 52069
