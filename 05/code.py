...
seats = [x for x in PathPlus("input.txt").read_lines() if x]


def calc_seat_id(seat: str) -> int:
	rows = list(range(128))
...
seat_ids = [calc_seat_id(seat) for seat in seats]
...
missing_seats = set(seat_ids) ^ set(range(min(seat_ids), max(seat_ids) + 1))
