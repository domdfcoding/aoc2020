...
def prepare_decks():
	players = [x for x in PathPlus("input.txt").read_text().split("\n\n") if x]
	player1 = list(map(int, filter(bool, players[0].split('\n')[1:])))
	player2 = list(map(int, filter(bool, players[1].split('\n')[1:])))

	return player1, player2


player1, player2 = prepare_decks()
...
while True:
	...
	player_1_card = player1.pop(0)
	player_2_card = player2.pop(0)

	if player_1_card > player_2_card:
		player1.append(player_1_card)
		player1.append(player_2_card)
	elif player_1_card < player_2_card:
		player2.append(player_2_card)
		player2.append(player_1_card)
	elif player_1_card == player_2_card:
		raise ValueError("Snap!")

total_score = 0

for idx, card in enumerate((player1 or player2)[::-1]):
	idx += 1
	total_score += (idx * card)

print("The total score is:", total_score)  # 31809
...
number on the card they just dealt, the winner of the round is determined by recursing into
a sub-game of Recursive Combat.
(For example, if player 1 draws the ``3`` card, and player 2 draws the ``7`` card, this would occur if
player 1 has at least ``3`` cards left and player 2 has at least ``7`` cards left,
...
(For example, if player 1 drew the ``3`` card, their deck in the sub-game
would be copies of the next three cards in their deck.)
...
class Game:
	...
	def __init__(self, player1: List[int], player2: List[int]):
		...
		self.previous_rounds: List[Tuple[List[int], List[int]]] = []

	def play(self):
		while True:

			if (self.player1, self.player2) in self.previous_rounds:
				# print("Player 1 wins!")
				break
			...
			self.previous_rounds.append((self.player1[:], self.player2[:]))

			player_1_card = self.player1.pop(0)
			player_2_card = self.player2.pop(0)

			if len(self.player1) >= player_1_card and len(self.player2) >= player_2_card:
				result = Game(self.player1[:player_1_card], self.player2[:player_2_card]).play()
				if result:
					self.player1.append(player_1_card)
					self.player1.append(player_2_card)
				else:
					self.player2.append(player_2_card)
					self.player2.append(player_1_card)
		...
		return bool(self.player1)


game = Game(*prepare_decks())
game.play()

total_score = 0

for idx, card in enumerate((game.player1 or game.player2)[::-1]):
	idx += 1
	total_score += (idx * card)

print("The total score is:", total_score)  # 32835
